...
namespace Koenigz.PerfectCulling
{
    public class PerfectCullingSceneColor : IDisposable
    {
        ...
        public PerfectCullingSceneColor(PerfectCullingBakeGroup[] groups, HashSet<Renderer> additionalOccluders, bool setupRenderers = true)
        {
            ...
            {
                // Duplicate m_additionalOccluders so we can manipulate it while iterating
                List<Renderer> additionalOccludersCopy = new List<Renderer>(m_additionalOccluders);

                foreach (Renderer renderer in additionalOccludersCopy)
                {
                    if (renderer == null)
                    {
                        continue;
                    ...
                    if (Application.isPlaying)
                    {
                        PrepareRenderer(newR, PerfectCullingConstants.ClearColor, new MaterialPropertyBlock());
                    }
                    else
                    {
                        m_additionalOccluders.Add(newR);
                    }
                }
            }
